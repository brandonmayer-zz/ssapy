...
from ssapy.strategies.condLocal import condLocalZero
...
def main():
    ...
    for itr, initBid, v, l in zip(xrange(vmat.shape[0]),initBids,vmat,lmat):
        ...
        bids[itr,:] = \
            condLocalZero(bundles,revenue,initBid,jointSamples,
    ...
    numpy.savetxt(os.path.join(args.odir,'condLocalZeroBids.txt'), bids)
    numpy.savetxt(os.path.join(args.odir,'condLocalZeroExpectedSurplus.txt'),es)
    
    with open(os.path.join(args.odir,'condLocalZeroStats.txt'),'w') as f:
